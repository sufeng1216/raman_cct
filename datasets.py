...
class SpectralDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = np.expand_dims(x, axis=0)  # 在前面加1维
        x = x.reshape(1,25,40) # 适配（1,1,25,40）
        if self.transform:
            x = self.transform(x)  # 取决于transform的参数，一些预处理操作
        return (x, y)
...
def spectral_dataloader(X_fn, y_fn, idxs=None, batch_size=10, shuffle=True,
    ...
    transform = transforms.Compose(transform_list)  # Compose用列表来串联多个transform操作
...
def spectral_dataloaders(X_fn, y_fn, n_train=None, p_train=0.8, p_val=0.1,
    ...
    max_idx=None):
    """
    Returns train, val, and test DataLoaders by splitting the dataset randomly.  # random?
    ...
    np.random.shuffle(idxs)  # 打乱顺序函数，原地操作
    if n_train is None: n_train = int(p_train * len(idxs))
    n_val = int(p_val * n_train)  # 0.8 * 0.1拿去验证？
    val_idxs, train_idxs = idxs[:n_val], idxs[n_val:n_train]  # val: 0.8*0.1, train: 0.8*0.9
    if n_test is None: test_idxs = idxs[n_train:]  # test: 0.2
    else: test_idxs = idxs[n_train:n_train+n_test]  # 参数不是None，有剩？
